...
import com.StreamTube.dtos.JwtResponse.JwtResponseBuilder;
...
public class AuthenticateController {
	...
	public ResponseEntity<?> createUser(@RequestBody UserDTO userDTO) {
		try {
			...
			// emailService.sendEmailForAccountCreated(user); // Uncomment if email service
			// is configured
	...
	public ResponseEntity<?> refreshToken(@RequestBody RefreshTokenRequest refreshTokenRequest) {
		...
		return refreshTokenService.findByToken(refreshTokenRequest.getRefreshToken())
				.map(refreshTokenService::verifyExpiration).map(refreshToken -> {
					...
					JwtResponse jwtResponse = new JwtResponse(newAccessToken, refreshToken.getRefreshToken(), username,
							userId);
					return ResponseEntity.ok(jwtResponse);
				}).orElseGet(
						() -> ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Refresh token not found or expired"));
	}
